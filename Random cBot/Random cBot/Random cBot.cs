...
/*
0,5|0,55|0,15|0|0,05|0,15|0,7|0,35|0,6|0,5
 
Mamy macierz z wagami dla wybranych wskaźników
Odpalamy zwykły back test. W trakcie back testu sprawdzamy oczekiwany wynik. Jeśli jest ok, 
to wg. wartości wskaźnika (skwantowanej), odpowiednio podbijana jest waga (wg. danego wsp. uczenia)
Na początek na przykładzie samego rsi
Analogicznie SL/TP na bazie skwantowanego ATR z wagami.
dla wag 0.1, 0.1 itd. rozkład jest całkowicie losowy
dokonujemy losowania wg. _random.NextDouble() np. 0.6 i teraz sprawdzamy jaki indeks ma skwantowany rsi wg wag.
Jeśli > to kup, jeśli < to sprzedaj
bez korelacji to nie ma sensu dlatego dodajemy 2 gi wskaźnik
teraz losujemy dla niego ww. schematu
następnie mamy jeszcze ważenie rsi+wsk2 daje nam ostateczny wynik. Do tego bias czy w ogóle dokonywać operacji.

Optymalizacja polega wyłącznie na iterowaniu X razy algorytmu na wybranym przedziale czasowym 
(dodać param Iter min 1 max 10000000 i odpalać dla wszystkich)
Wagi będą zapisywane na dysk i przy każdym uruchomieniu odczytywane (json)

Dla SL/TP może być trudno określić co byłoby lepsze lub gorsze, ale mozemy w przypadku przegranej sprawdzać min wartość
i jakoś modyfikować ATR.*/